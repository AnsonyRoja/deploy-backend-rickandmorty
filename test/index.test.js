...
describe("Test de RUTAS", () => {
    ...
    describe("GET /rickandmorty/login", () => {
        ...
        it("Responde con un objeto con la propiedad access en true si la informacion del usuario es valida", async () => {
            ...
            const response = await request.get('/rickandmorty/login?email=Ansonyrojas2015@gmail.com&password=123asd');
            expect(response.body).toEqual(access);

        })
        it("Responde con un objeto con la propiedad access en true si la informacion del usuario no es valida", async () => {

            const response = await request.get('/rickandmorty/login?email=Ansonyrojas2015@gmail.com&password=123dassd');
            access.access = false;
            expect(response.body).toEqual(access);
    ...
    describe("DELETE /rickandmorty/fav/:id", () => {
        ...
        it("Si el ID solicitado no existe, deberia retornar un arreglo con todos los favoritos", async () => {
            ...
            const response = await request.delete('/rickandmorty/fav/2wqw');
            expect(response.body.length).toBe(2);
